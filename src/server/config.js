...
export default function(configType, baseConfig, configDir) {
  ...
  if (fs.existsSync(storybookCustomAddonsPath)) {
    console.info('=> Loading custom addons config.');
  ...
  if (!fs.existsSync(customConfigPath)) {
    console.info('=> Using default webpack setup based on "Create React App".');
  ...
  if (typeof customConfig === 'function') {
    console.info('=> Loading custom webpack config (full-control mode).');
    return customConfig(config, configType);
  }
  console.info('=> Loading custom webpack config (extending mode).');
  return {
    ...customConfig,
